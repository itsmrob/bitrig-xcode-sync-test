...
struct BOMPersistenceService {
  private let session: URLSession
  private let endpoint: URL

  init(
    session: URLSession = .shared,
    endpoint: URL = URL(string: "https://web-production-81f39.up.railway.app/api/bom")!
  ) {
    self.session = session
    self.endpoint = endpoint
  }

  func loadBOM() async throws -> BOMDocument? {
    ...
    do {
      return try JSONDecoder().decode(BOMDocument.self, from: data)
    } catch {
      throw BOMPersistenceError.decodingFailed
    }
  }

  func saveBOM(_ document: BOMDocument) async throws {
    var request = URLRequest(url: endpoint)
